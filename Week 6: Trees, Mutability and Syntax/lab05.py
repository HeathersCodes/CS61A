def factors_list(n):
    ...
    """
    all_factors = []
    for i in range(1, n//2+1): #notes: not 0
        if n % i == 0: # % or //
...
def distance(city_a, city_b):
    ...
    return sqrt(diff_lat**2 + diff_lon**2)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    if distance(position, city_b) <= distance(position, city_a):
...
def sprout_leaves(t, leaves):
    ...
    if is_leaf(t):
        ...
        new = []
        for branch in branches(t):
            new.append(sprout_leaves(branch, leaves))
        return tree(label(t), new)
