...
def choose(paragraphs, select, k):
    ...
    para = [paragraphs[i] for i in range(len(paragraphs)) if select(paragraphs[i])]
    return para[k] if k < len(para) else ''
    # END PROBLEM 1
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def func(x):
        para_lst = split(remove_punctuation(lower(x)))
        for el in topic:
            for ele in para_lst:
                if el == ele:
                    return True
        return False #loop is over, return False
    return func
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    if typed_words == [] or reference_words == []:
        if reference_words == typed_words:
            return 100.0
        else:
            return 0.0 #do not forget
    i = 0
    num = 0
    x = min(len(typed_words), len(reference_words))
    for i in range(x):
        if typed_words[i] == reference_words[i]:
            num += 1
    return 100 * num / len(typed_words)
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    'testing'
    """
    min = diff_function(typed_word, valid_words[0], limit)
    store = 0
    # BEGIN PROBLEM 5
    for i in range(len(valid_words)):
        ...
        if diff_function(typed_word, valid_words[i], limit) < min:
            min = diff_function(typed_word, valid_words[i], limit)
            store = i
    if min > limit:
        return typed_word
    else:
        return valid_words[store]
    # END PROBLEM 5
    #####But I find the greater answer in github####
    '''
...
def feline_flips(start, goal, limit):
    ...
    def helper(amount, start, goal, limit):
        if amount > limit:
            return limit + 1
        if len(start) == 0:
            return amount + abs(len(start)-len(goal))
        if len(goal) == 0:
            return amount + abs(len(start)-len(goal))  
        else:
            if start[0] == goal[0]:
                return helper(amount, start[1:], goal[1:], limit)
            else:
                return helper(amount+1, start[1:], goal[1:], limit)
...
def minimum_mewtations(start, goal, limit):
    ...
    def helper(amount, start, goal, limit):
        if amount > limit:
            ...
            if start[0] == goal[0]:
                return helper(amount, start[1:], goal[1:], limit)
            add = helper(amount+1, start, goal[1:], limit)
            remove = helper(amount+1, start[1:], goal, limit)
            substitude = helper(amount+1, start[1:], goal[1:], limit)
            return min(add, remove, substitude)
    return helper(0, start, goal, limit)
    # https://www.youtube.com/watch?v=rsCEsdg4rjc
...
def report_progress(sofar, prompt, user_id, upload):
    ...
    if sofar == [] or prompt == []:
        if prompt == sofar:
            ...
            return 0.0 #do not forget
    num = 0
    x = min(len(sofar), len(prompt))
    for i in range(x):
        if sofar[i] == prompt[i]:
            num += 1
        else:
            break
    dic = {'progress': num / len(prompt), 'id': user_id}
    upload(dic)
    return dic['progress']
    # END PROBLEM 8
...
def time_per_word(words, times_per_player):
    ...
    for i in range(len(times_per_player)):
        lst = []
        index = times_per_player[i] #good to use in the following process
        for j in range(len(index)-1):
            lst.append(index[j+1]-index[j]) 
        result.append(lst)
