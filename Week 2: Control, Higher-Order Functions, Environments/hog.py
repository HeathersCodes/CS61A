...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1 because have to finish the loop
    have_num_one = False
    result = 0
    while num_rolls > 0:
        curr = dice()
...
def take_turn(num_rolls, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    # BEGIN PROBLEM 3 not dice=six_sided
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    # BEGIN PROBLEM 5 if using while score0 < goal or score1 < goal, running out
    while 1: 
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    # BEGIN PROBLEM 7 cannot not find the bug about nonlocal
    def say(score0, score1):
        if who == 1:
            prev = score1
            curr = prev - last_score
        elif who == 0:
            prev = score0
            curr = prev - last_score
        last, high = prev, 0

        if curr > running_high:
            print(f"{curr} point(s)! That's a record gain for Player {who}!")
            high = curr
        else:
            high = running_high
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    index = 0
    while count < 11:
        func = make_averaged(roll_dice, trials_count) #1st arg
        score = func(count, dice)
        if score > outcome:
            outcome = score #do not forget
            index = count
        count += 1
    return index
    # END PROBLEM 9
...
def hog_pile_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    effect. 
    It also returns 0 dice if it gives at least CUTOFF points.
